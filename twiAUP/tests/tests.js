...
describe("Single User API tests", function() {
    ...
    describe("timeline", function() {
        ...
        it("can be posted with articles", function(alldone) {
            this.timeout(5000);
            $.ajax('../api/post.php?title=hello%20world&flit=my%20life%20is%20cool&coorx=0&coory=0')
                .success(function(data) {
                    var response = JSON.parse(data);
                    expect(response).to.deep.equal({
                        'status': 1,
        ...
        it("should how another article after posted", function(alldone) {
            this.timeout(5000);
            $.ajax('../api/post.php?title=goodbye%20world&flit=my%20life%20is%20uncool&coorx=0&coory=0')
                .success(function(data) {
                    var response = JSON.parse(data);
                    expect(response).to.deep.equal({
                        'status': 1,
