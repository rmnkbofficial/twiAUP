...
<div class="row" style='border-bottom: 1px solid #000; margin-bottom: 10px; padding-top:20px;'>
	<div class="row" style='padding-left: 10px; padding-right: 10px; padding-top: 5px;'>
		<div class="col-md-10">
			<h2>Most Popular Tag Pairs</h2>
		</div>
	</div>
</div>
<?php
$num_result = 0;
$resp = get_most_popular_tag_pairs($dbh, 5);

if($resp['status'] == 1) {
	error_log("Get most popular tag pairss!");
	$tagpairs = $resp['tagpairs'];
	for($i = 0; $i < count($tagpairs); $i++) {
		html_tag_pair($dbh, $tagpairs[$i]);
		$num_result++;
	}
	if($num_result==0){
		echo '<p>There appears to be no tag pair here</p>';
	}
}
